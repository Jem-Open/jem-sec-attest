...
- [x] T009 [US1] Wire fire-and-forget evidence generation into `app/api/training/[tenant]/evaluate/route.ts` — after successful session update to "passed" or "exhausted" status (after audit logging), call `generateEvidenceForSession(storage, tenantId, sessionId).catch((err) => console.error("Evidence generation failed:", err))` using the same storage instance; import from `@/evidence/evidence-generator`
- [x] T010 [US1] Wire fire-and-forget evidence generation into `app/api/training/[tenant]/abandon/route.ts` — after successful session update to "abandoned" status (after audit logging), call `generateEvidenceForSession(storage, tenantId, sessionId).catch((err) => console.error("Evidence generation failed:", err))` using the same storage instance; import from `@/evidence/evidence-generator`
