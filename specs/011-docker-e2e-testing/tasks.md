...
- [x] T007 Create `Dockerfile` — two-stage build: stage 1 `node:20-alpine AS builder` (install pnpm, `pnpm install --frozen-lockfile`, `pnpm build`, copy `public/` and `.next/static/` into `.next/standalone/`); stage 2 `node:20-alpine AS runner` (install `curl`, create `nodejs:1001` user, copy `.next/standalone/`, set `NODE_ENV=production PORT=3000 HOSTNAME=0.0.0.0`, `USER nodejs`, `EXPOSE 3000`, `HEALTHCHECK --interval=30s --timeout=10s --start-period=40s --retries=3 CMD curl -f http://localhost:3000/api/health || exit 1`, `CMD ["node", "server.js"]`)
...
- [x] T010 [US1] Create `docker/compose.yml` — three services: `postgres` (`postgres:16-alpine`, env vars from `.env.docker`, healthcheck `pg_isready -U postgres -d jem_attest` every 10s/5 retries/30s start, network `jem_local`); `dex` (`dexidp/dex:v2.37.0`, ports `5556:5556` and `5558:5558`, volume mount `./dex/config.yaml:/etc/dex/config.yaml:ro`, command `dex serve /etc/dex/config.yaml`, healthcheck `curl http://localhost:5558/healthz/ready` every 10s/3 retries/15s start, depends_on postgres healthy, network `jem_local`); `app` (build context `..` dockerfile `../Dockerfile`, port `3000:3000`, env_file `../.env.docker`, environment `NODE_ENV=production`, `DATABASE_URL=postgres://postgres:postgres@postgres:5432/jem_attest`, `ACME_OIDC_ISSUER_URL=http://dex:5556/dex`, `ACME_OIDC_CLIENT_ID=jem-app`, `ACME_OIDC_REDIRECT_URI=http://localhost:3000/api/auth/acme-corp/callback`, healthcheck `curl http://localhost:3000/api/health` every 30s/3 retries/40s start, depends_on postgres healthy AND dex healthy, network `jem_local`); volumes `postgres_data`; networks `jem_local: driver: bridge`
